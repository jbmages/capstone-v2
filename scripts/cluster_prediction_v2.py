...
from huggingface_hub import upload_file, HfApi, HfFolder

class ClusterPredictor:
    def __init__(self, data, scoring, cluster_type='KMeans_Cluster', test_size=0.2, use_subset=True, subset_size=100000, upload_to_huggingface=True):
        self.data = data.copy()
        ...
        self.upload_to_huggingface = upload_to_huggingface
    ...
    def upload_models_to_huggingface(self, repo_id="your-username/cluster_predictor", folder='models'):
        if self.upload_to_huggingface:
            for name, model in self.models.items():
                model_path = os.path.join(folder, f"{name.replace(' ', '_').lower()}_model.joblib")
                upload_file(
                    path_or_fileobj=model_path,  # Model file
                    path_in_repo=f"{name.replace(' ', '_').lower()}_model.joblib",  # Name in Hugging Face repo
                    repo_id=repo_id,
                    repo_type="model",
                )
            # Upload scaler
            ...
            upload_file(
                path_or_fileobj=scaler_path,  # Scaler file
                path_in_repo="scaler.joblib",  # Name in Hugging Face repo
                repo_id=repo_id,
                repo_type="model",
            )
            print("Models uploaded to Hugging Face.")
        else:
            print("Upload to Hugging Face skipped for testing purposes.")
...
scoring = pd.read_csv('path_to_your_scoring_file.csv')

predictor = ClusterPredictor(data, scoring)
predictor.train_all()
predictor.save_models()
predictor.upload_models_to_huggingface()
predictor.plot_model_accuracies()
predictor.stepwise_feature_analysis(top_n=5, model_type='random_forest')
